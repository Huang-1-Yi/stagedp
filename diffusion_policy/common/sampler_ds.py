"""
第一版：固定长度序列（sequence_length）
第二版：各字段独立配置（key_horizon+key_down_sample_steps）
​​数据采样技术​​
第一版：直接内存切片
第二版：动态插值（线性插值+球面旋转插值）
​​机器人特性支持​​
第二版特有：
# 抓取状态检测
gripper_width = replay_buffer['robot0_gripper_width'][:, 0]
before_first_grasp = (gripper_width < threshold)

# 重复帧概率
if before_first_grasp and random.random() < repeat_frame_prob:
    result[key][:-1] = result[key][-1:]  # 复制最后一帧
​动作构造逻辑​​
第二版自动构建动作：
if 'action' not in replay_buffer:
    # 从机器人状态构造动作
    actions = [eef_pos, eef_rot, gripper_width]
    self.replay_buffer['action'] = np.concatenate(actions)
"""
