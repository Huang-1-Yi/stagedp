...
class PoseTrajectoryInterpolator:
    ...
    def reset(self, new_pose: np.ndarray, reset_time: float = None):
        """完全重置插值器到新位姿和时间"""
        if reset_time is None:
            reset_time = time.monotonic()  # 使用绝对时间
        
        # 强制重建插值器结构
        self._times = np.array([reset_time])
        self._poses = np.array([new_pose])
        ...
        if len(self._times) == 1:
            ...
            self.rot_interp = st.Slerp(
                self._times,
                st.Rotation.from_rotvec(self._poses[:,3:])
            )