...
--dataset
类型: 字符串(str)
​​说明​​: 输入HDF5文件路径,必须是一个有效的HDF5文件,包含需要转换的原始数据。
​​是否必需​​: 是

2.
--output_name​​
​​类型​​: 字符串(str)
​​说明​​: 输出HDF5文件名。注意:文件将保存在与输入文件相同的目录下。
​​是否必需​​: 是

3.
--num_procs​​
​​类型​​: 整数(int)
​​说明​​: 并行进程数。默认值为1,即单进程。实际进程数会被自动调整为不超过待处理演示数(demos)的最小值。
​​默认值​​: 1

4.
--gpu_ids​​
​​类型​​: 整数列表(nargs='*',即可接受多个参数)
​​说明​​: 使用的GPU ID列表。如果指定,进程将按照指定的分配策略(默认为轮询)分布在这些GPU上。如果没有提供,则不会进行显式GPU分配。
​​可选值​​: 可以是任意多个GPU索引(例如,0,1,2,3)。
​​默认值​​: None(不指定)

5.
--procs_per_gpu​​
​​类型​​: 整数列表(nargs='*')
​​说明​​: 每个GPU上分配的进程数列表。必须与--gpu_ids的长度相同,且所有GPU分配的进程数之和必须等于--num_procs。如果未指定,则采用轮询方式分配。
​​示例​​: 如果指定--gpu_ids 0 1 --procs_per_gpu 3 2,则表示GPU0分配3个进程,GPU1分配2个进程。
​​注意​​: 如果进程总数与分配之和不等,会进行调整(尽量从分配多的GPU上减少进程)。
​​默认值​​: None(不指定)

6.
--camera_names
​​类型​​: 字符串列表(nargs='+',表示一个或多个字符串)
​​说明​​: 使用的相机名称。可以指定一个或多个相机名称。如果不指定(即省略该参数),则不会使用图像观测。
​​可选值​​: 取决于环境中的相机配置,常见的有：
'frontview', 'birdview', 'agentview', 'sideview', 'agentview_full', 'robot0_robotview', 'robot0_eye_in_hand'
​​默认值​​: 空列表(不使用图像观测)
...
10.
--shaped​​
​​类型​​: 标志(action='store_true',即不需要参数)
​​说明​​: 如果指定,则使用密集奖励(dense rewards)。如果不指定,则使用稀疏奖励。
​​默认值​​: False(不指定则为False)

11.
--depth​​
​​类型​​: 标志(action='store_true')
​​说明​​: 如果指定,则每个相机将提供深度观测。注意:指定此标志时必须同时指定--camera_names(即至少一个相机)。
​​默认值​​: False(不指定则为False)

补充说明:
除了以上列出的参数,代码中还有其他的参数,如:

--start: 可选,指定从哪个演示索引开始处理(整数,默认为None,即从0开始)。

--n: 可选,处理多少个演示后停止(整数,默认为None,即处理所有)。

--copy_rewards: 标志,如果指定,则从源文件中复制奖励而不是重新计算(默认为False)。

--copy_dones: 标志,如果指定,则从源文件中复制done信号(默认为False)。

--use-actions: 标志,使用动作回放而不是逐个状态重置(默认为False)。

--exclude-next-obs: 标志,在数据集中不包含next_obs(默认为False)。

--compress: 标志,使用gzip压缩存储观测值(默认为False)。

--generative-textures: 标志,对robosuite环境使用生成纹理(默认为False)。

--randomize-cameras: 标志,随机化相机位姿(默认为False)。

--no-slack: 标志,禁用Slack通知(默认为False)。
"""
