...
from diffusion_policy.model.common.utils import (
    ImgEmbedding,
    LangEmbedding,
    miniLangEmbedding,
    Fusion,
...
class ControlNet(nn.Module):
    ...
    def forward(self, x, obs, lang, control_input, time):
        ...
        for resnet, resnet2, downsample in self.downs:
            ...
            x = downsample(x)
        x = self.mid_block1(x, t)
        x = self.mid_block2(x, t)#action下采样 --> x
        ...
        x_hat = self.copy_mid_block2(x_hat, t) #prior+action进行下采样 -->x_hat

        """
        add feature for the middle blocks
        """
        x_hat = self.mid_controlnet_block(x_hat)
        x = x + x_hat  #两个采样进行叠加 -->x
        ...
        for resnet, resnet2, upsample in self.ups:
            ...
            x = upsample(x)
            # print(x.shape)
        x_out = self.final_conv(x) #-->对x进行上采样
        return x_out
