...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        model = timm.create_model(
            ...
            pretrained=True,
            pretrained_cfg_overlay=dict(file='/home/hy/umi/timm/vit_base_patch16_clip_224.openai/pytorch_model.bin'),
    ...
    def forward(self, obs_dict):
        # 输入维度打印
        # print("\n[TimmObsEncoder] Input shapes:")
        # for key, value in obs_dict.items():
        #     print(f"  {key}: {value.shape}")
        """
        [TimmObsEncoder] Input shapes:
            camera_0: torch.Size([1, 3, 240, 320])
            camera_1: torch.Size([1, 3, 240, 320])
            robot_eef_pose: torch.Size([1, 6])
            robot_gripper: torch.Size([1, 1])
        [TimmObsEncoder] Input shapes:
        camera_0: torch.Size([64, 3, 240, 320])
        camera_1: torch.Size([64, 3, 240, 320])
        robot_eef_pose: torch.Size([64, 6])
        robot_gripper: torch.Size([64, 1])
        """
        features = list()
    ...
    def output_shape(self):
        # return torch.Size([1, 3086])
        example_obs_dict = dict()
        obs_shape_meta = self.shape_meta['obs']
        for key, attr in obs_shape_meta.items():
            shape = tuple(attr['shape'])
            this_obs = torch.zeros(
                (1, self.n_obs_steps) + shape, 
                dtype=self.dtype,
                device=self.device)
    ...
    #     for key, attr in obs_shape_meta.items():
    #         shape = tuple(attr['shape'])
    #         print(f"key: {key}, shape: {shape}, self.n_obs_steps: {self.n_obs_steps}")
    #         # RGB输入 (1, 2, C, H, W)
    #         # 低维输入 (1, self.n_obs_steps, D)
    #         this_obs = torch.zeros(
    #             (1, self.n_obs_steps) + shape,
    #             dtype=torch.float32,
    #             device=self.device)
    #         print(f"example_obs_dict[{key}].shape: {this_obs.shape}")
    #         example_obs_dict[key] = this_obs
    ...
    #     # 验证输出维度是否为3086
    #     expected_shape = torch.Size([1, 3086])
    #     if example_output.shape != expected_shape:
    #         # 计算实际维度
    #         actual_dim = example_output.shape[1]
            
    #         # 分析可能的原因
    #         error_msg = (
    #             f"输出形状不匹配: 预期 {expected_shape}, 实际 {example_output.shape}\n"
    #             "可能原因:\n"
    #         )
            
    #         # 计算预期维度
    #         rgb_count = len([k for k, v in obs_shape_meta.items() if v.get('type') == 'rgb'])
    #         lowdim_count = len([k for k, v in obs_shape_meta.items() if v.get('type') == 'low_dim' and not v.get('ignore_by_policy', False)])
            
    #         # 每个时间步的维度
    #         per_step_dim = rgb_count * 768 + lowdim_count
    #         expected_dim = per_step_dim * self.n_obs_steps

    #         error_msg += f"- 预期维度: {expected_dim} (每步 {per_step_dim} × {self.n_obs_steps} 步)\n"
    #         error_msg += f"- 实际维度: {actual_dim}\n"
            
    #         # 检查特征聚合方式
    #         if self.feature_aggregation not in [None, 'all_tokens']:
    #             error_msg += f"- 特征聚合方式 '{self.feature_aggregation}' 可能增加了额外维度\n"
