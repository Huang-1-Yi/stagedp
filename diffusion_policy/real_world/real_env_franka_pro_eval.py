...
from diffusion_policy.common.interpolation_util import get_interp1d, PoseInterpolator    # 机器人轨迹平滑插值
...
class RealEnvFranka:
    def __init__(self, 
            ...
            max_obs_buffer_size = 60,                   # 最大观察缓冲区大小
        ...
        realsense = MultiRealsense(
            ...
            capture_fps     =video_capture_fps,         # 视频捕获帧率=记录帧率
    ...
    def end_episode(self):
        ...
        if self.obs_accumulator is not None:                        # 如果观察累加器不为空，结束记录
            ...
            for cam_idx, accum in self.image_accumulators.items():
                print(f"camera_{cam_idx} timestamps: {len(accum.timestamps)}")
