...
class RealPushTImageDataset(BaseImageDataset):
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        # ---------- Low‑dim ----------
        SKIP_NORM_KEYS = {'stage'}          # 不归一化，也不放进 normalizer
        print(f"归一化 'lowdim' 字段: {self.lowdim_keys}")

        for key in self.lowdim_keys:
            if key not in available_keys:
                print(f"⚠️ 缺失 low‑dim 字段 {key}")
                continue

            if key in SKIP_NORM_KEYS:
                print(f"跳过归一化，使用恒等 normalizer: {key}")

                # 1. 先准备占位 stats（mean=0, std=1）
                dummy_stats = {
                    'mean': np.zeros((1,), dtype=np.float32),
                    'std':  np.ones((1,),  dtype=np.float32)
                }

                # 2. offset / scale 也用 1‑D 数组
                offset_arr = np.zeros((1,), dtype=np.float32)   # [0.]
                scale_arr  = np.ones((1,),  dtype=np.float32)   # [1.]

                normalizer[key] = SingleFieldLinearNormalizer.create_manual(
                    offset_arr,          # offset
                    scale_arr,           # scale
                    dummy_stats          # input_stats_dict
                )
                continue

            print(f"归一化字段: {key}, 形状: {self.replay_buffer[key].shape}")
            normalizer[key] = SingleFieldLinearNormalizer.create_fit(
                self.replay_buffer[key].astype(np.float32)
            )
