...
### ddpm demo=400 512降维度到256  50次测试，成功率(epoch100开始 ，ckpt=90-94) 选定为baseline
训练：
...
### ddim demo=400 512降维度到256  50次测试，成功率() 
训练：
python train_sim.py --config-name=robomimic_train_dp_diffusion_unet_abs_sample_channel_ddim task_name=stack_d1 n_demo=400
8月27日 
测试：
