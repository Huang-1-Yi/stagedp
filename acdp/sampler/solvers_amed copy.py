...
def init_hook(net, class_labels=None):
    """简化版的hook初始化，适用于动作生成领域"""
    class DummyHook:
        def remove(self):
            """空方法，什么也不做"""
            pass
    
    return [], DummyHook()  # 返回一个对象实例
...
def get_amed_prediction(AMED_predictor, t_cur, t_next, net, unet_enc_out, use_afs, batch_size):
    ...
    time_features = torch.cat([
        t_cur.unsqueeze(1),
        t_next.unsqueeze(1),
        (t_next - t_cur).unsqueeze(1),
        torch.log(t_cur + 1e-7).unsqueeze(1)
    ], dim=1)
    
    # 传递给预测器
    output = AMED_predictor(time_features)
...
def amed_sampler(
    ...
    **kwargs
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):                # 0, ..., N-1
        ...
        hook.remove()  # 现在可以安全调用 remove 方法
