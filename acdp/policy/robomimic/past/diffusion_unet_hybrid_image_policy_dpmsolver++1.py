...
class DiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    def conditional_sample(self, 
        ...
        # 对于 DPM Solver，我们需要使用不同的方式
        # 这里使用 DPM Solver 的多步采样
        # Use DPM-Solver multi-step sampling
        # Check scheduler type and sample accordingly
        model_outputs = None
        for i, t in enumerate(scheduler.timesteps):
            ...
            # 3. Manage model outputs history for multi-step solver
            if model_outputs is None:
                model_outputs = [model_output]
            else:
                model_outputs.append(model_output)
                # Keep the list length not exceeding the solver order
                if len(model_outputs) > scheduler.config.solver_order:
                    model_outputs.pop(0)

            # 4. compute previous image: x_t -> x_t-1
            # Prepare keyword arguments for the scheduler step, including generator if provided
            step_kwargs = {}
            if generator is not None:
                step_kwargs["generator"] = generator
            # Also pass any additional kwargs that might be needed
            step_kwargs.update(kwargs) 

            trajectory = scheduler.step(
                model_output, t, trajectory, 
                model_outputs=model_outputs if len(model_outputs) > 1 else None,
                **step_kwargs
            ).prev_sample
