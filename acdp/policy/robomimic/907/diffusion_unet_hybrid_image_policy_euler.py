...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def conditional_sample(self, 
        ...
        # 从配置中获取参数
        sigma_min = self.sampler_kwargs.get('sigma_min', 0.002)
        sigma_max = self.sampler_kwargs.get('sigma_max', 80.0)
        schedule_type = self.sampler_kwargs.get('schedule_type', 'polynomial')
        schedule_rho = self.sampler_kwargs.get('schedule_rho', 7)
        afs_enabled = self.sampler_kwargs.get('afs', True)
        num_steps = self.num_inference_steps

        # 生成时间表
        t_steps = self.get_schedule(num_steps, sigma_min, sigma_max, device=device, 
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            # 判断是否使用解析第一步
            use_afs = afs_enabled and i == 0
            if use_afs:
                # 解析第一步
                d_cur = trajectory / torch.sqrt(1 + t_cur**2)
            else:
                # 预测模型输出
                model_output = model(trajectory, t_cur, 
                                    local_cond=local_cond, global_cond=global_cond)
                d_cur = model_output

            # Euler更新步骤
            trajectory = trajectory + (t_next - t_cur) * d_cur
    ...
    def get_schedule(self, num_steps, sigma_min, sigma_max, device=None, 
        ...
        if schedule_type == 'polynomial':
            ...
            t_steps = (-t_steps).exp()
        elif schedule_type == 'discrete' and net is not None:
            # 实现离散时间表
    ...
    def compute_loss(self, batch):
        ...
        # === 手动实现加噪过程 ===
        # 采样随机时间步（连续值）
        t = torch.rand(batch_size, device=trajectory.device)  # [0, 1]之间
        sigma_min = 0.002
        sigma_max = 80
        
        # 计算噪声水平（对数均匀采样）
        log_sigma = t * math.log(sigma_max) + (1 - t) * math.log(sigma_min)
        sigma = torch.exp(log_sigma)
        
        # 确保sigma有正确的维度用于广播
        sigma = sigma.view(-1, 1, 1)  # 扩展为 [B, 1, 1]

        # 采样噪声
        noise = torch.randn_like(trajectory)
        
        # 加噪（扩散过程）
        noisy_trajectory = trajectory + noise * sigma
        ...
        # 预测噪声残差
        pred = self.model(noisy_trajectory, sigma.squeeze(),  # 传入连续时间步
            local_cond=local_cond, global_cond=global_cond)
