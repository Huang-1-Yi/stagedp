...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch):
        ...
        print(f"[compute_loss] 开始计算损失")
        print(f"[compute_loss] 批次键: {list(batch.keys())}")

        # 归一化输入
        assert 'valid_mask' not in batch
        nobs = self.normalizer.normalize(batch['obs'])
        nactions = self.normalizer['action'].normalize(batch['action'])
        print(f"[compute_loss] 归一化观察形状: {nactions.shape}, 归一化动作形状: {nactions.shape}")

        if self.rot_aug and self.rot_randomizer is not None:
            print(f"[compute_loss] 应用旋转增强")
            nobs, nactions = self.rot_randomizer(nobs, nactions)
            print(f"[compute_loss] 增强后观察形状: {nactions.shape}, 增强后动作形状: {nactions.shape}")
        
        batch_size = nactions.shape[0]
        horizon = nactions.shape[1]
        print(f"[compute_loss] 批次大小: {batch_size}, 时间步长: {horizon}")
        ...
        if self.obs_as_global_cond:
            print(f"[compute_loss] 使用观察作为全局条件")
            # reshape B, T, ... to B*T
            ...
            print(f"[compute_loss] 使用观察作为局部条件")
            # reshape B, T, ... to B*T
            ...
            print(f"[compute_loss] 条件数据形状: {cond_data.shape}")
            print(f"[compute_loss] 轨迹形状: {trajectory.shape}")


        # 生成掩码 (保持不变)
        condition_mask = self.mask_generator(trajectory.shape)
        print(f"[compute_loss] 条件掩码形状: {condition_mask.shape}")
        print(f"[compute_loss] 轨迹形状: {trajectory.shape}")
        ...
        print(f"[compute_loss] 时间步形状: {timesteps.shape}, 值范围: [{timesteps.min().item()}, {timesteps.max().item()}]")
        
        # 计算alpha和sigma
        sigma_min = self.sampler_kwargs['sigma_min']
        sigma_max = self.sampler_kwargs['sigma_max']
        print(f"[compute_loss] 噪声范围: sigma_min={sigma_min}, sigma_max={sigma_max}")
        
        # 将时间步映射到[0,1]区间
        t_ratio = timesteps.float() / (self.num_inference_steps - 1)
        print(f"[compute_loss] 时间比率形状: {t_ratio.shape}, 值范围: [{t_ratio.min().item()}, {t_ratio.max().item()}]")
        
        
        # 计算噪声水平 (对数均匀采样)
        sigma = torch.exp(t_ratio * math.log(sigma_max) + 
                         (1 - t_ratio) * math.log(sigma_min))
        print(f"[compute_loss] 噪声水平形状: {sigma.shape}, 值范围: [{sigma.min().item()}, {sigma.max().item()}]")
        
        # 计算alpha和缩放因子
        alpha = 1.0 / torch.sqrt(1.0 + sigma**2)
        scale = alpha * sigma
        print(f"[compute_loss] alpha形状: {alpha.shape}, 值范围: [{alpha.min().item()}, {alpha.max().item()}]")
        print(f"[compute_loss] scale形状: {scale.shape}, 值范围: [{scale.min().item()}, {scale.max().item()}]")

        # 确保alpha和scale有正确的维度用于广播
        alpha = alpha.view(-1, 1, 1)
        scale = scale.view(-1, 1, 1)
        print(f"[compute_loss] 调整后alpha形状: {alpha.shape}")
        print(f"[compute_loss] 调整后scale形状: {scale.shape}")

        # 采样噪声
        noise = torch.randn_like(trajectory)
        print(f"[compute_loss] 噪声形状: {noise.shape}")
        
        # 加噪 (扩散过程)
        noisy_trajectory = alpha * trajectory + scale * noise
        print(f"[compute_loss] 加噪后轨迹形状: {noisy_trajectory.shape}")
        ...
        print(f"[compute_loss] 损失掩码形状: {loss_mask.shape}")

        # 应用条件约束
        noisy_trajectory[condition_mask] = cond_data[condition_mask]
        print(f"[compute_loss] 应用条件后轨迹形状: {noisy_trajectory.shape}")
        
        # 预测噪声残差
        print(f"[compute_loss] 输入模型: noisy_trajectory={noisy_trajectory.shape}, timesteps={timesteps.shape}")
        pred = self.model(noisy_trajectory, timesteps, 
            local_cond=local_cond, global_cond=global_cond)
        print(f"[compute_loss] 模型预测形状: {pred.shape}")
        ...
        print(f"[compute_loss] 目标形状: {target.shape}")

        # 计算损失
        loss = F.mse_loss(pred, target, reduction='none')
        print(f"[compute_loss] 原始损失形状: {loss.shape}")
        
        loss = loss * loss_mask.type(loss.dtype)
        print(f"[compute_loss] 加权后损失形状: {loss.shape}")
        
        loss = reduce(loss, 'b ... -> b (...)', 'mean')
        print(f"[compute_loss] 平均后损失形状: {loss.shape}")
        
        loss = loss.mean()
        print(f"[compute_loss] 最终损失值: {loss.item()}")
        return loss
