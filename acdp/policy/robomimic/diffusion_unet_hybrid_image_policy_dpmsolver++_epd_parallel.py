...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def conditional_sample(self, 
        ...
        get_epd_prediction = self.get_epd_prediction
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            unet_enc_out, hook = init_hook(model, class_labels)   # 捕获瓶颈特征
            ...
            if use_afs:
                ...
                denoised = model(x_cur, t_cur, local_cond=local_cond, global_cond=global_cond)
            ...
            if step_idx is not None:
                r_s, scale_dir_s, scale_time_s, weight_s = get_epd_prediction(predictor,
                                                                    step_idx, 
                                                                    model, unet_enc_out, 
                                                                    use_afs, batch_size=x_next.shape[0])
            else:
                r_s, scale_dir_s, scale_time_s, weight_s = get_epd_prediction(predictor, 
                                                                    i, 
                                                                    model, unet_enc_out, 
            ...
            denoised_t_mid_flat = model(x_mid_flat, t_param, local_cond=local_cond, global_cond=global_cond)
            denoised_t_mid_flat[condition_mask] = condition_data[condition_mask]
