...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def conditional_sample(self, 
        ...
        for i, (t_c, t_n) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            # 添加 Epsilon Scaling
            if eps_scaler != 1.0:
                pred_eps = (x_next - x0_hat_c) / t_c
                pred_eps = pred_eps / eps_scaler
                denoised = x_next - pred_eps * t_c
            ...
            # slope at stage
            x0_hat_mid = model(x_pred, t_mid, local_cond=local_cond, global_cond=global_cond)
            x0_hat_mid[condition_mask] = condition_data[condition_mask]
