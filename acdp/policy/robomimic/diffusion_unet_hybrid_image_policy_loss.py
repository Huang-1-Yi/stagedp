...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch):
        ...
        betas = self.noise_scheduler.betas.to(device)[timesteps]  # [bsz]
        
        # 计算 sigma_t^2 = beta_t
        sigma_t_sq = betas  # 根据 iDDPM 论文，sigma_t^2 = beta_t
        
        # 计算权重因子：1 / sigma_t^2
        weights = 1.0 / sigma_t_sq  # [bsz]
        
        # 确保权重数值稳定（避免除以零）
        weights = torch.clamp(weights, max=1e4)
        
        # 扩展权重维度以匹配预测结果的形状
        weights = weights.view(-1, *([1]*(noise.dim()-1)))  # [bsz, 1, 1, ...]
        # ====================================================
