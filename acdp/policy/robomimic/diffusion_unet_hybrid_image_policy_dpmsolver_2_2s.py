...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def conditional_sample(self, 
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            for i, (t_c, t_n) in enumerate(zip(t_s[:-1],t_s[1:])):
                ...
                if use_afs:
                    ...
                    d_cur = model(x_cur, t_c, local_cond=local_cond, global_cond=global_cond)
                    d_cur[condition_mask] = condition_data[condition_mask]

                # 4.2 阶段点（几何插值，r∈(0,1]；r=0.5 为中点）
                t_mid = (t_n ** r) * (t_c ** (1 - r))
                ...
                d_mid = model(x_pred, t_mid, local_cond=local_cond, global_cond=global_cond)
                d_mid[condition_mask] = condition_data[condition_mask]
        ...
        if denoise_to_zero:
            ...
            eps_final = model(x_next, t_next, local_cond=local_cond, global_cond=global_cond)
            eps_final[condition_mask] = condition_data[condition_mask]
