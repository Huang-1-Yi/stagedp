...
class ZarrInspector:
    ...
    def generate_additional_fields(self, overwrite=False):
        """
        功能11：生成额外的数据字段
        - 3维的robot0_eef_pos
        - 6维的robot0_eef_rot_axis_angle
        - 10维的action
        - 匹配的robot0_eef_pos_wrt_start
        - robot0_eef_rot_axis_angle_wrt_start
        """
        try:
            logger.info("===== 开始生成额外数据字段 =====")
            ...
            # 检查必要的基础数据是否存在
            required_keys = ['robot_eef_pose', 'robot_gripper', 'action']
            for key in required_keys:
                if key not in data_group:
                    logger.error(f"缺少必要数据字段: {key}")
                    return False
            ...
            for name, shape in new_datasets.items():
                if name in data_group:
                    if overwrite:
                        logger.info(f"覆盖已存在的字段: {name}")
                        del data_group[name]
                    else:
                        logger.info(f"字段已存在: {name}，跳过创建")
                        continue
                
                logger.info(f"创建新字段: {name} [形状: {shape}]")
                data_group.zeros(
                    name=name,
                    shape=(data_group['robot_eef_pose'].shape[0],) + shape,
                    chunks=(1000,) + shape,
                    dtype='float32'
                )
            ...
            for ep_idx in trange(len(starts)):
                ...
                # 获取当前episode的基础数据
                eef_pose = data_group['robot_eef_pose'][start_idx:end_idx+1]
                gripper = data_group['robot_gripper'][start_idx:end_idx+1]
                action_7d = data_group['action'][start_idx:end_idx+1]
                ...
                # 将旋转转换为6D表示
                rotations_6d = self.axis_angle_to_6d(rotations_aa)
                
                # 创建10维动作 (位置3D + 旋转6D + 夹爪1D)
                action_10d = np.zeros((ep_length, 10), dtype=np.float32)
                action_10d[:, :3] = action_7d[:, :3]  # 位置
                action_10d[:, 3:9] = self.axis_angle_to_6d(action_7d[:, 3:6])  # 旋转(6D)
                action_10d[:, 9] = action_7d[:, 6]  # 夹爪
                
                # 计算相对于起始点的位姿
                start_pos = positions[0]
                start_rot_6d = rotations_6d[0]
                
                # 计算相对位置
                rel_positions = positions - start_pos
                
                # 计算相对旋转 (在6D空间中)
                rel_rotations_6d = np.zeros_like(rotations_6d)
                for i in range(ep_length):
                    rel_rotations_6d[i] = self.relative_rotation_6d(start_rot_6d, rotations_6d[i])
                
                # 存储计算结果
                data_group['robot0_eef_pos'][start_idx:end_idx+1] = positions
                data_group['robot0_eef_rot_axis_angle'][start_idx:end_idx+1] = rotations_6d
                data_group['action_10d'][start_idx:end_idx+1] = action_10d
                data_group['robot0_eef_pos_wrt_start'][start_idx:end_idx+1] = rel_positions
                data_group['robot0_eef_rot_axis_angle_wrt_start'][start_idx:end_idx+1] = rel_rotations_6d
            
            logger.info("===== 额外数据字段生成完成 =====")
            return True
        
        except Exception as e:
            logger.exception(f"生成额外字段时出错: {str(e)}")
            return False

    def axis_angle_to_6d(self, aa_vectors):
        """
        将轴角旋转转换为6D旋转表示
        """
        from scipy.spatial.transform import Rotation
        rotations = Rotation.from_rotvec(aa_vectors)
        matrices = rotations.as_matrix()
        
        # 6D表示：旋转矩阵的前两列
        return matrices[:, :, :2].reshape(aa_vectors.shape[0], 6)

    def relative_rotation_6d(self, rot6d_start, rot6d_current):
        """
        计算两个6D旋转之间的相对旋转
        """
        from scipy.spatial.transform import Rotation
        ...
        mat_start = rot6d_to_matrix(rot6d_start)
        mat_current = rot6d_to_matrix(rot6d_current)
        
        # 计算相对旋转矩阵
        rel_mat = np.matmul(np.linalg.inv(mat_start), mat_current)
        
        # 将相对旋转转换回6D表示
        return rel_mat[:, :2].flatten()
...
if __name__ == "__main__":
    ...
    zarr_path = "/home/hy/Desktop/dp_0314/data/button_grb_0730/replay_buffer.zarr"
    stages_output_path = "./data/button_grb_0730/episode_stages"
    stages_output_dir="./data/button_grb_0730/episode_images"
    try:
        # 创建检查器
        ...
        #     支持覆盖已存在字段（可选）
        print("\n====== 功能11: 生成额外数据字段 ======")
        success = inspector.generate_additional_fields(overwrite=True)
        
        if success:
            print("额外字段生成成功!")
            # 检查新字段
            inspector.inspect_key("data/robot0_eef_pos")
            inspector.inspect_key("data/robot0_eef_rot_axis_angle")
            inspector.inspect_key("data/action_10d")
            inspector.inspect_key("data/robot0_eef_pos_wrt_start")
            inspector.inspect_key("data/robot0_eef_rot_axis_angle_wrt_start")
        else:
            print("额外字段生成失败")
